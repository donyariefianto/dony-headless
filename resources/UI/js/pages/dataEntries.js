...
export const setupDataEntries = () => {
    ...
    const renderGrid = (documents) => {
        ...
        const gridHtml = documents.map(doc => `
            <div class="grid-item">
                <h3>${doc.name}</h3>
                <p>${doc.description}</p>
    ...
    const createSubformRow = (fields, tableName) => {
        let rowHtml = `<div class="subform-row">`;
        fields.forEach(subField => {
            let subFieldHtml = `<div class="subform-field-group">`;
            subFieldHtml += `<label>${subField.label}</label>`;
            switch (subField.type) {
                case 'select':
                    subFieldHtml += `<select name="${tableName}[]_${subField.name}"></select>`;
                    break;
                case 'number':
                    subFieldHtml += `<input type="number" name="${tableName}[]_${subField.name}">`;
                    break;
                case 'calculated':
                    subFieldHtml += `<input type="text" name="${tableName}[]_${subField.name}" value="0.00" data-formula="${subField.options?.formula || ''}" readonly>`;
                    break;
                default:
                    subFieldHtml += `<input type="text" name="${tableName}[]_${subField.name}">`;
                    break;
            }
            subFieldHtml += `</div>`;
    ...
    const setupFormListeners = (formConfig) => {
        ...
        document.querySelectorAll('.add-row-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                const subformRowsContainer = e.currentTarget.closest('.subform-container').querySelector('.subform-rows');
        ...
        form.addEventListener('submit', async (e) => {
            ...
            if (!validateForm(formConfig, form)) {
                formMessageContainer.innerHTML = '<p style="color: red;">Ada kesalahan dalam pengisian form.</p>';
                return;
            }
    ...
    const collectFormData = (formConfig, form) => {
        ...
        formConfig.subforms.forEach(subform => {
            if (subform.label) {
                ...
                const subformRows = form.querySelectorAll(`.subform-container[data-table-name="${subform.table_name}"] .subform-row`);
                subformRows.forEach(row => {
                    const rowData = {};
                    subform.fields.forEach(subField => {
                        const el = row.querySelector(`[name="${subform.table_name}[]_${subField.name}"]`);
                        if (el) {
    ...
    const updateAllCalculatedFields = (formConfig) => {
        ...
        formConfig.subforms.forEach(subform => {
            const subformRows = document.querySelectorAll(`.subform-container[data-table-name="${subform.table_name}"] .subform-row`);
            subformRows.forEach(row => {
                subform.fields.forEach(subField => {
                    if (subField.type === 'calculated') {
                        const formula = subField.options?.formula;
                        if (formula) {
                            const result = calculateFormulaInRow(formula, row, subform.table_name);
                            row.querySelector(`[name="${subform.table_name}[]_${subField.name}"]`).value = result;
                        }
                    }
                });
            });
    ...
    const calculateFormulaInRow = (formula, row, tableName) => {
        ...
        for (const part of parts) {
            if (['+', '-', '*', '/'].includes(part)) {
                ...
                const value = parseFloat(row.querySelector(`[name="${tableName}[]_${part}"]`)?.value || 0);
                expression += value;
            }
        }
