...
function handleExtraOptions(typeContainer, optionsData = null, fieldType = null) {
  ...
  if (selectedType === 'select') {
    ...
    extraOptionsContainer
      .querySelector('.select-mode')
      ?.addEventListener('change', (e) => toggleSelectModeOptions(e.currentTarget))
...
function buildFieldWidget(fieldData = null) {
  ...
  wrapper
    .querySelector('.toggle-collapse-btn')
    ?.addEventListener('click', (e) => toggleCollapse(e.currentTarget))
...
function buildSubformWidget(subformData = null) {
  ...
  wrapper
    ...
    ?.addEventListener('click', (e) => toggleCollapse(e.currentTarget))
  wrapper
    .querySelector('.add-subfield-btn')
    ?.addEventListener('click', (e) => addSubField(e.currentTarget))
    ...
    fields: Array.from(formPanel.querySelectorAll('#main-fields .field-item')).map((div) => {
      ...
      if (type === 'select') {
        ...
        if (extra) {
          ...
          options.mode = mode
          if (mode === 'static') {
            options.values = (extra.querySelector('.select-static-values')?.value || '')
              .split(',')
              .map((v) => v.trim())
          } else {
            options.relation = {
...
function attachFormGeneratorEventListeners() {
  ...
  formPanel
    ...
    ?.addEventListener('click', () => generateSchema(formPanel))
  ...
  formPanel.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const url = formId
        ? `${BASE_API_URL}/configuration/formbuilder/update/${formId}`
        : `${BASE_API_URL}/configuration/formbuilder/create`
  ...
  if (window.currentFormData) {
    ...
    formPanel.querySelector('#form-submit-label').value =
      window.currentFormData.submit_label || 'Simpan'
